...
$1/32bit/Dockerfile : $1/64bit/Dockerfile
	...
	sed 's/ENTRYPOINT\ \[\"/ENTRYPOINT\ \[\"linux32\",\ \"/g' "$1/Dockerfile.1.tmp" > "$1/Dockerfile.2.tmp"           && mv "$1/Dockerfile.2.tmp" "$1/Dockerfile.1.tmp"
	sed 's/\ .\/miniconda.sh/\ linux32\ .\/miniconda.sh/g' "$1/Dockerfile.1.tmp" > "$1/Dockerfile.2.tmp"              && mv "$1/Dockerfile.2.tmp" "$1/Dockerfile.1.tmp"
	sed 's/\ conda\ /\ linux32\ conda\ /g' "$1/Dockerfile.1.tmp" > "$1/Dockerfile.2.tmp"                              && mv "$1/Dockerfile.2.tmp" "$1/Dockerfile.1.tmp"
	...
	sed 's/\/opt\/python\/cp27-cp27m\/bin/linux32\ \/opt\/python\/cp27-cp27m\/bin/g' <"$1/Dockerfile.1.tmp" > "$1/Dockerfile.2.tmp" && mv "$1/Dockerfile.2.tmp" "$1/Dockerfile.1.tmp"
